...
class iso _UriHostCanBeIp6WithZoneId is UnitTest
  ...
  fun apply(h: TestHelper) ? =>
    ...
    h.assert_error(_ConstructorOf.uri("s://[::%25]"), "Empty zone ID")
    h.assert_error(_ConstructorOf.uri("s://[::%en1]"), "Not encoded separator")
...
class iso _UriUsesEntireRepresentation is UnitTest
  fun name(): String =>
    "uri/Uri uses entire representation"
...
class iso _UriConvertedToStringProducesInitialRepresentation is UnitTest
  fun name(): String =>
    "uri/Uri converted to string produces initial representation"

  fun apply(h: TestHelper) ? =>
    let representations = [
      "s:", "s:/", "s://",
      "s://host", "s://@host", "s://user@host", "s://host:42",
      "s://127.0.0.1", "s://[A::B]", "s://[::%25en1]", "s://[v1337.address]",
...
class iso _UriConvertedToStringOmitsEmptyPassword is UnitTest
  fun name(): String =>
    "uri/Uri converted to string omits empty password"
...
class iso _UriConvertedToStringHidesPassword is UnitTest
  fun name(): String =>
    "uri/Uri converted to string hides password"
...
class iso _UriConvertedToUnsafeStringRevealsPassword is UnitTest
  fun name(): String =>
    "uri/Uri converted to unsafe string reveals password"
...
class iso _UriConvertedToStringOmitsEmptyPort is UnitTest
  fun name(): String =>
    "uri/Uri converted to string omits empty port"
